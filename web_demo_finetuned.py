...
def generate_text_with_image(
    input_text, image, history=[], request_data=dict(), is_zh=True
):
    input_para = {
        "max_length": 2048,
        "min_length": 50,
    ...
    input_data = generate_input(
        input_text, image, history, input_para, image_is_encoded=False
    )
...
def request_model(input_text, temperature, top_p, image_prompt, result_previous):
    ...
    if image_prompt is None:
        if is_zh:
            ...
            result_text.append(
                (input_text, "Image empty! Please upload a image and retry.")
            )
    ...
    try:
        answer = generate_text_with_image(
            input_text, image, result_text.copy(), request_para, is_zh
        )
